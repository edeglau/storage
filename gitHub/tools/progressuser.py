...
class attributeSwapper(object):
        ...
        EG:"/usr/people/<user>/joint4.txt"
            ...
            * Step 2: pressing save will create .txt files that will contain the animation
            ...
            cmds.button (label='Load atts', p='listBuildButtonLayout', w=150, command = lambda *args:self.openAttributesWindow(optionPath=cmds.optionMenu(self.foundPath, q=1, v=1)))
        ...
        def _choser_group_window(self, getListAttr):
            ...
            cmds.button (label='Force Selected', p='listBuildButtonLayout', w=150, command = lambda *args:self.open_genAttributefolderWindow(folderType=cmds.optionMenu(self.attributeFirstSel, q=1, v=1)))
        ...
        def _apply_found_att(self, folderType):
            ...
            for filename in getallfiles:
                ...
                isolateitemName=isolateitemName.split(':')[1]
        ...
        def open_genAttributefolderWindow(self, getPathCustom, getPath):
            if getPath=="custom":   
                getPath=getPathCustom
            else:
                getPath=getPath             
        ...
        def _choser_gen_group_window(self, getListAttr):
            ...
            for each in getListAttr:
                cmds.menuItem( label=each)
            cmds.gridLayout('listBuildButtonLayout', p='title1', numberOfColumns=3, cellWidthHeight=(148, 20))
            cmds.button (label='Ok', p='listBuildButtonLayout', w=150, command = lambda *args:self.forceAtt(getPath=cmds.optionMenu(self.attributepath, q=1, v=1)))
        ...
        def _save_anim_heirarchy(self, fileName, getPathCustom, optionPath):   
            ...
            folderType=optionPath+'/'+fileName
            if os.path.exists(folderType):
                pass
            else:
        ...
        def _save_gen_sel_nim_heirarchy(self, fileName, radioType, folderPath): 
            selObj=ls(sl=1, fl=1, sn=1)
            if len(selObj)<1:
                print "select something"
                return
            ...
            folderType=folderPath+'/'+fileName
            if os.path.exists(folderType):
                pass
            else:
                self.makeFolder(folderType)
