...
            except:
                cut_shouldbe_in_value = 0.0        if 'CUT_OUT' in line:
...
    #     if not os.path.exists(gotten_path): os.makedirs(gotten_path)
...
    #     # cmds.playblast(clearCache = 0, startTime = wk_strt_value, endTime = wk_out_value, filename = filepathname, c = "jpg", format="image", offScreen=1, percent=100, quality=100, sequenceTime = 0, showOrnaments=1, viewer=0, widthHeight=[2156, 1212])
    #     time.sleep(1.3)
    #     command = "rv "+ gotten_path
...
        scn_name=str(cmds.file(q=1, location=1)).split('/')[-1].split(".")[0]
        gotten_name=str(cmds.file(q=1, location=1)).split('/')[-1].split(".")[0]+"-playblast-half1712x903_vdf8"
        gotten_path = "{}/edeglau/maya/images/temp/{}".format(folder_strt, M_USER, gotten_name)
        print (gotten_path)
        sel_NumMn=cmds.playbackOptions(q=1, min=1)
        sel_NumMn_reset=cmds.playbackOptions(q=1, min=1)+1
        cmds.currentTime(sel_NumMn)
...
        newName, cachFolder = self.checkCacheFile()        # cmds.cacheFile(pcf=cachFolder, f=newName, format = 'OneFile')
        # cmds.cacheFile(pcf=cachFolder, f=newName, format = 'OneFile', sch=0, cnd = 'hairSystem')
...
            interroName = interroName+"_"
        newName, cachFolder = self.checkCacheFile()
        # cmds.cacheFile(pcf=cachFolder, f=newName, format = 'OneFile')
        # cmds.cacheFile(pcf=cachFolder, f=newName, format = 'OneFile', sch=0, cnd = 'hairSystem')
        # command = 'doCreateNclothCache 5{"2", "1", "10", "OneFilePerFrame", "1", "%s", "1", "%s", "0", "rename", "0", "1", "1", "0", "1", "mcx"};' %( cachFolder, newName)
        print ("got here")
