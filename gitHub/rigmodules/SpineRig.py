...
#         clstrSplineCtrl=clstrSplineCnt[1:-1]
        clstrCtrl=[]
        scaleWorldMatrix = cmds.xform("spine01_guide", q=True, r=1, s=True)
        ...
        FKCtrl=[]
        FKTorsoJoint=clstrSplineCtrl.split("_guide")[0]+"FK_jnt"  
        name="Torso_FK_Ctrl"
        grpname="Torso_FK_grp"   
        size=14*scaleWorldMatrix
        colour=6
        nrx=0
        nry=1
        nrz=0    
        ...
        FKCtrl.append(name)      
        cmds.parentConstraint("Torso_FK_Ctrl",FKTorsoJoint , mo=1)             
        
        name="UpperBody_Ctrl"
        grpname="UpperBody_grp"    
        size=20*scaleWorldMatrix
        colour=22
        nrx=0
        nry=1
        nrz=0    
        ...
        cmds.circle(n="Master_Ctrl", r=30*scaleWorldMatrix, nrx=0, nry=1, nrz=0)
        ...
        cmds.circle(n="Main_Ctrl", r=25*scaleWorldMatrix, nrx=0, nry=1, nrz=0)
        cmds.setAttr("Main_CtrlShape.overrideEnabled", 1)
        cmds.setAttr("Main_CtrlShape.overrideColor", 17)
        getClass.buildGrp("Main_Ctrl")
        
        
        cmds.circle(n="Main_offset_Ctrl", r=27*scaleWorldMatrix, nrx=0, nry=1, nrz=0)
        ...
        if len(FKCtrl)>1:
            ...
            for eachctrl in xrange(len(FKCtrl) - 1):
                ...
                cmds.parent(getParentgrp, current_item)             
#         else:
        ...
        cmds.parent("Chest_FK_grp", lastFKCtrl)
        cmds.parent(firstFK, "Hips_Ctrl")        
        #cmds.parent(lastClstr, "Chest_IK_Ctrl")        
        cmds.parent("Hips_grp","UpperBody_Ctrl")
        cmds.parent("ChestIK_grp","UpperBody_Ctrl")
        cmds.parent("UpperBody_grp","Main_Ctrl")
        cmds.parent("spine01_jnt","Hips_Ctrl")
        cmds.parent("spine01FK_jnt","Hips_Ctrl")
        cmds.parent("spine01IK_jnt","Hips_Ctrl") 
        #cmds.parent("spine02_Clst_jnt","UpperBody_Ctrl")
...
#             getClusterChunk=[seq[i:i+size] for i  in range(0, len(seq), size)]
        ...
        size=3
        getMiddleClusters= [getMidClstr[each:each + size] for each in range(0, len(getMidClstr), size)]
        firstlist=(getMiddleClusters[0])
        firstlist= reversed(firstlist)
        #beginning
        BucketValue=[]
        getPercentile=100/len(getMiddleClusters[0])
        for key, value in enumerate(getMiddleClusters[0]):
            percValue= (key+1)*getPercentile*.01
            BucketValue.append(percValue)
        for eachCluser, weighted in map(None, firstlist, BucketValue):
            cmds.parentConstraint( "Hips_Ctrl", eachCluser, mo=1, w=weighted) 
        #end
        BucketValue=[]
        getPercentile=100/len(getMiddleClusters[1])            
        for key, value in enumerate(getMiddleClusters[1]):
            percValue= (key+1)*getPercentile*.01
            BucketValue.append(percValue)
        for eachCluser, weighted in map(None,getMiddleClusters[1], BucketValue):
            print eachCluser, weighted
            cmds.parentConstraint( "Chest_IK_Ctrl", eachCluser, mo=1, w=weighted)  
        ...
        for key, value in enumerate(getMiddleClusters[0]):
            percValue= (key+1)*getPercentile*.01
            BucketValue.append(percValue)
        for eachCluser, weighted in map(None, getMiddleClusters[0], BucketValue):
            cmds.parentConstraint( "Torso_IK_Ctrl", eachCluser, mo=1, w=weighted)         
        BucketValue=[]
        getPercentile=100/len(getMiddleClusters[1])            
        for key, value in enumerate(getMiddleClusters[1]):
            percValue= (key+1)*getPercentile*.01
            BucketValue.append(percValue)
        for eachCluser, weighted in map(None,reversed(getMiddleClusters[1]), BucketValue):
            cmds.parentConstraint( "Torso_IK_Ctrl", eachCluser, mo=1, w=weighted)   

#             
#         #getMidClstr=[(each) for each in cmds.listRelatives("Torso_IK_Ctrl", ad=1) if "Clst" in each and cmds.nodeType(each)=="joint"]
#         getMidClstr=cmds.ls("spine*_Clst_jnt_grp")
#         #cmds.orientConstraint("Chest_FK_Ctrl", fkSpine[-1:], mo=1)
# #         for i in range(size):
# #             getMiddleClusters.append(getMidClstr[int(round(i*splitsize)):int(round((i+1)*splitsize))])
...
# #         getMiddleClusters=map(None, *[iter(getMidClstr)]*3)
# #         print getMiddleClusters
