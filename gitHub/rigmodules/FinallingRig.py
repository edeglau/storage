    ...
    def make_cluster_ctrl(self, arg=None):
        ...
        for each in selObj:
            ...
            try:
                ...
                print "must select a vert"
            position, rotation=getClass.locationXForm(each)
            try:
                ...
                if getSkinCluster:
                    ...
                    getClass.buildJoint(getName, position, rotation)
#                     cmds.skinCluster(skinID, e=1, ai=getName+"_jnt", lw=1, wt=0) 
                ...
                getClass.buildJoint(getNewName, position, rotation)
                try:
...
#                     cmds.skinCluster(each, getNewName+"_jnt", tsb=1) 
    ...
    def cluster_shape(self,each, name, grpname, position, rotation):
            ...
            getClass.CCCircle(name, grpname, num0, num1, num2, num3, position, rotation, colour) 
             
    def circle_shape(self, each, name, grpname, position, rotation):
        nrx, nry, nrz=getClass.fetchDirection()
        size=5 
        colour=22
        getClass.buildCtrl(each, name, grpname, position, rotation, size, colour, nrx, nry, nrz)       
...
#                     cmds.skinCluster(skinID, e=1, ai=getName+"_jnt", lw=1, wt=0) 
                ...
                getClass.buildJoint(thisIsAVariableName, position, rotation)
                try:
                    cmds.skinCluster(each, thisIsAVariableName+"_jnt", tsb=1) 
