...
class plotter_UI(object):
    ...
    def plotter_mayaUI(self):
        ...
        window = cmds.window(winName, title=winTitle, tbm=1, w=550, h=800 )
    ...
    to the face or vertex of your choice
        ...
        cmds.rowColumnLayout  (' selectArrayRow ', nr=1, w=300, h=500)
        ...
        cmds.button (label='Locate Mass', p='txvaluemeter', command = lambda *args:self.locator_selected_mass())
        ...
        cmds.rowColumnLayout  ('vertex_align', nc=1, w=300, h=500, p='BRow')
        # cmds.rowColumnLayout( 'vertex_align', p = 'BRow', numberOfColumns=1 ) 
        cmds.text ('Vertice Align', p='BRow', bgc=[0.55, 0.6, 0.6])
        # self.zip = cmds.checkBox("Meet")
        cmds.gridLayout('strngthlayout', p='BRow', numberOfColumns=2, cellWidthHeight=(80, 18))          
        cmds.text ('Strength', p='BRow')
        self.strength = cmds.textField( w=40, h=25, p='BRow', text="1.0")        
        cmds.gridLayout('biaslayout', p='BRow', numberOfColumns=2, cellWidthHeight=(80, 18))          
        cmds.text ('Bias', p='biaslayout')
        self.bias = cmds.textField( w=40, h=25, p='BRow', text="0.5")  
        cmds.gridLayout('txvaluemeter', p='BRow', numberOfColumns=3, cellWidthHeight=(60, 18))          
        cmds.text ('Drift', p='BRow')
        self.direction = cmds.optionMenu( label='Axis', w=60, p='BRow')
        for each in getDir:
            cmds.menuItem( label=each)
        self.amount = cmds.textField( w=40, h=25, p='BRow', text="0.0")        
        cmds.button (label='Align', p='BRow', command = lambda *args:self._offset_verts(strength=cmds.textField(self.strength, q=1, text=1), amount=cmds.textField(self.amount, q=1, text=1), direction=cmds.optionMenu(self.direction, q=1, v=1), biases=cmds.textField(self.bias, q=1, text=1)))
        cmds.button (label='Align_mass', p='BRow', command = lambda *args:self._offset_verts_mass(strength=cmds.textField(self.strength, q=1, text=1), amount=cmds.textField(self.amount, q=1, text=1), direction=cmds.optionMenu(self.direction, q=1, v=1), biases=cmds.textField(self.bias, q=1, text=1)))
    ...
    def _offset_verts_mass(self, strength, amount, direction, biases):
        self.space_vert(strength, amount, direction, biases)
    ...
    def space_vert(self, strength, amount, direction, bias):
        ...
        for leadingVert, followVert in map(None, firstpart, secondpart):
            cmds.select(leadingVert, r=1)
            cmds.select(followVert, add=1)
            getfirst=cmds.ls(sl=1)[0]
            getsecond=cmds.ls(sl=1)[1]
            transform=cmds.xform(getfirst, q=True, ws=1, t=True)
            transform_follvert=cmds.xform(getsecond, q=True, ws=1, t=True)
            #calc x
    ...
    # def locator_select_verts(self, arg=None):
    #     selObj=cmds.ls(sl=1, fl=1)
    #     transform=cmds.xform(selObj,  q=1, wd=1, t=1)
    #     getLoc=cmds.spaceLocator()
    #     cmds.xform(getLoc[0], t=(transform[0], transform[1], transform[2]))
    #     return getLoc[0]
