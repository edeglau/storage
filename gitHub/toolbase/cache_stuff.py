...
import os, mrig_pyqt, sys, glob, shutil
import getpass
import webbrowser
import re
...
# from mrig_pyqt import QtGui, QtCore
...
try:
    ...
    getScenePathWorkPath=mc.file(q=1, location=1)
    ...
    getCachePath=mc.workspace(listWorkspaces=1)[1]+"/cache/nCache"
...
getLowRange = mc.playbackOptions(q=1, min=1)
getHiRange = mc.playbackOptions(q=1, max=1)
...
class save_gen(QtWidgets.QMainWindow):
    ...
    def initUI(self):
        ...
        self.playblast_work_checkbox.setChecked(1)
        ...
        self.playblast_range_checkbox.setChecked(0)
        ...
        self.playblast_head_field = QtWidgets.QLineEdit(str(wk_strt_value - 1))
        ...
        self.playblast_radiobox.addWidget(self.playblast_head_field, 2,4,1,1)
        
        self.playblast_toe_lbl = QtWidgets.QLabel("end")
        ...
        self.playblast_toe_field = QtWidgets.QLineEdit(str(wk_out_value + 1))
    ...
    def blast_and_play(self):
        ...
        if len(getScene)>0:
            ...
            "you need to save scene to blast and play"
        newName, cachFolder=self.checkCacheFile()
        wk_strt_value, wk_out_value = self.get_range_for_play()
        if not os.path.exists(getCacheimages): os.makedirs(getCacheimages)
        filepathname = getCacheimages+'/'+filename_string+'/'+filename_string
        mc.playblast(clearCache = 0, startTime = wk_strt_value, endTime = wk_out_value, filename = filepathname, c = "jpg", format="image", offScreen=1, percent=100, quality=100, sequenceTime = 0, showOrnaments=1, viewer=0, widthHeight=[2156, 1212])
        time.sleep(1.3)
        command = "rv "+ getCacheimages
    ...
    def createCacheselected(self):
        getNSystems = mc.ls(sl=1, fl=1)
        self.createCache_callup(getNSystems)
        
        
    def createHCache(self):
        grabsystem = mc.ls(type = "hairSystem")
        ...
        # command = 'doCreateNclothCache 5 { "2", "1", "10", "OneFilePerFrame", "1", "","1","","0", "add", "0", "1", "1","0","1","mcx" };'
    ...
    def createNCloth(self):
        if len(mc.ls(sl=1)) <1:
            ...
            allgrabs = mc.ls(sl=1)
            grabonlysystem = [(each) for each in allgrabs if mc.nodeType(each) == "nCloth"]
            grabtransform = [(each) for each in allgrabs if mc.nodeType(each) == "transform"]
            grabmoresystem = [(each) for each in allgrabs if mc.listRelatives(each, c=1, type = "nCloth") ]
        ...
        newName, cachFolder = self.checkCacheFile()
        # mc.cacheFile(pcf=cachFolder, f=newName, format = 'OneFile')
        # mc.cacheFile(pcf=cachFolder, f=newName, format = 'OneFile', sch=0, cnd = 'hairSystem')
        # command = 'doCreateNclothCache 5{"2", "1", "10", "OneFilePerFrame", "1", "%s", "1", "%s", "0", "rename", "0", "1", "1", "0", "1", "mcx"};' %( cachFolder, newName)
        command = 'doCreateNclothCache 5{"2", "1", "10", "OneFile", "0", "%s", "1", "", "0", "new", "0", "1", "1", "0", "1", "mcx"};' %( cachFolder)
        maya.mel.eval(command)   
    ...
    def createNClothV2(self):
        print "cloths"
        if len(mc.ls(sl=1)) <1:
            grabsystem = mc.ls(type = "nCloth")
        else:
            grabsystem = mc.ls(sl=1)
        print grabsystem
        mc.select(grabsystem, r=1)
        for each in grabsystem:
            getCache = [(nodes) for nodes in mc.listHistory(each) if mc.nodeType(nodes) == "cacheBlend"]
    ...
    def getCache(self):
        getNsystems = mc.ls(type = "nCloth")
        mc.select(getNSystems)
    ...
    def createCache_callup(self, getNSystems):
        ...
        if len(getNSystems)>0:
            for the_sel in getNSystems:
                if mc.nodeType(the_sel) == "mesh":
                    try:
                        grabMesh = [(each) for each in mc.listRelatives(the_sel, ad=1, type = "mesh") if "Orig" not in each]
                    except:
                        print "can't"
                        pass
                if mc.nodeType(the_sel) != "nCloth":
                    try:
                        grabmoresystem = [(each) for each in mc.listRelatives(the_sel, ad=1, type = "nCloth") ]
                    except:
                        pass
                if mc.nodeType(the_sel) != "hairSystem":
                    try:
                        grabmorechildHairsystem = [(each) for each in mc.listRelatives(the_sel, ad=1, type = "hairSystem") ]        
                    except:
                        pass
        ...
        # if mc.nodeType(getNSystems[0])=="hairSystem":
            print "hair here"
            mc.select(collectionHair)
            # command = 'doCreateNclothCache 5{"2", "1", "10", "OneFile", "0", "", "1", "%s", "0", "rename", "0", "1", "1", "0", "1", "mcx"};'%newName
            command = 'doCreateNclothCache 5{"2", "1", "10", "OneFile", "0", "%s", "1", "", "0", "new", "0", "1", "1", "0", "1", "mcx"};' %( cachFolder)
            maya.mel.eval(command)
        if len(collectionCloth)>0:
        # if mc.nodeType(getNSystems[0])=="nCloth":
            print " cloth here "
            mc.select(collectionCloth)
            # command = 'doCreateNclothCache 5{"2", "1", "10", "OneFile", "0", "", "0", "%s", "0", "rename", "0", "1", "1", "0", "1", "mcx"};'%newName
            command = 'doCreateNclothCache 5{"2", "1", "10", "OneFile", "0", "%s", "1", "", "0", "new", "0", "1", "1", "0", "1", "mcx"};' %( cachFolder)
            maya.mel.eval(command)          
        if len(grabMesh)>0:
            print "mesh here"
        # if mc.nodeType(mc.ls(sl=1)[0])=="transform":
            mc.select(grabMesh)
