...
        except:
            pass        self.cust_checked_two = QtWidgets.QCheckBox()
        self.value_label = QtWidgets.QLabel("Amount to animate")
...
        cust_check_qry_three = self.cust_checked_three        if cust_check_qry_three.isChecked():
...
            else:
                print( "Nothing selected. skipping")
                pass    def make_arbitrary_title(self):
        """
...
                        cmds.setKeyframe(each, at=key, v=value_hi, time=(active_tm_frm_hi))
                        cmds.setKeyframe(each, at=key, v=find_def_val, time=(end_tm_frm))
                cmds.currentTime(end_tm_frm)            #create the title
            for each in trgt_ctrlrs:
                title_set = each+"."+key
...
            cmds.currentTime(end_tm_frm)        elif fnd_dir == "circle":
            for each in trgt_ctrlrs:
                for key, value in cust_dict.items():
                    get_cur = cmds.currentTime(q=1)
                    size_area=value[0]*-1
...
        try:
            cmds.select(new_name_annot, r=1)
            cmds.hyperShade(assign=str(create_shade_node))        except:
            pass
        try:
            cmds.parent(new_name_annot, annot_title_grp)
        except:
...
                    active_tm_frm_lo = get_cur+gethalf
                    active_tm_frm_hi = get_cur+gethalf+gethalf
                    end_tm_frm = get_cur+get_frames                    #keyframe the object
                    if mirror == True:
                        print ("mirror True")
...
                    cmds.setKeyframe(new_name_annot, at="visibility", v=0.0, time=(end_tm_frm))    
                    cmds.currentTime(end_tm_frm)
            cmds.select(trgt_ctrlrs, r=1)        elif fnd_dir == "fig_eight":
            for each in trgt_ctrlrs:
                try:
                    cmds.sets(each, add=ctrl_set_name)
                except:
...
        val07 = [-0.701, 0.0, 1.799,0.0, -40, 0.0]        time_frame = get_cur+get_portion*7
        tm_frm_coll.append(time_frame)
...
        time_frame = get_cur+get_portion*16        tm_frm_coll.append(time_frame)
...
            cmds.setAttr("{}.rotateY".format(anim_loc_grp), 90)        elif "Z" in directional_plane:
            print ("Z")
            cmds.setAttr("{}.rotateX".format(anim_loc_grp), -90)
...
        tm_frm_coll.append(time_frame)
        self.transform_anim(anim_loc, val06, time_frame)
        val07 = [1.083, 0.0, 0.0,0.0, -40, 0.0]        time_frame = get_cur+get_portion*7
        tm_frm_coll.append(time_frame)
        self.transform_anim( anim_loc, val07, time_frame)
...
        cmds.setAttr("{}.scaleZ".format(anim_loc_grp), size_area)
        self.control_anim(tm_frm_coll, get_obj, anim_loc[0])
        cmds.delete(anim_loc_grp)    def create_orbit_anim_loc(self, get_cur, get_portion, get_obj, size_area):
        """
...
        self.transform_anim( anim_loc, val07, time_frame)
        val08 = [0.766, 0.0, -0.766,0.0, 0.0, 0.0]
        time_frame = get_cur+get_portion*8        tm_frm_coll.append(time_frame)
        self.transform_anim(anim_loc, val08, time_frame)
...
            cmds.setKeyframe(cur_obj, at=attr, v=val, time=(time_frame))    def control_anim(self, tm_frm_coll, get_obj, anim_loc):
        """
        This passes the animation from the animated locator (created on the creat anim function) onto the group of controller(s)
            Args:
...
        inst_win = get_val_frm(get_val_hi, def_val, trgt_obj, attr_channel)    def test_blendShape(self):
        """
...
            pass
        else:
            cmds.sets(n=set_name, co=3)        cmds.sets(lst_sg_node, add=set_name)  
