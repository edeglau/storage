...
__version__ = '2.0 - now has checkers, store gamut and enhance!'
...
            'uvbypeterdackers-pixelsham.png',
            'fi_uv_1024_radial_adfrac_by_fisholith.jpg',
...
maphome ='/net/Method_Transfer/departments/CFX/UVmaps/'
...
        self.prnt_verbose_button.clicked.connect(lambda: self.apply_rgb())
...
        self.u_tile_button.clicked.connect(lambda: self.u_tile_force())
        self.tile_setup_layout.addWidget(self.u_tile_button,0,0,1,1)
        self.vtile_button = QtWidgets.QPushButton('V tile')
        self.vtile_button.clicked.connect(lambda: self.v_tile_force())
...
        self.u_tile_line_edit.textChanged.connect(self.text_set_u_tile_slider)
...
        self.v_tile_line_edit.textChanged.connect(self.text_set_v_tile_slider)
...
        self.uv_rot_line_edit.textChanged.connect(self.text_set_rot_slider)
...
        self.u_trns_button.clicked.connect(lambda: self.u_trns_force())
...
        self.u_trns_line_edit.textChanged.connect(self.text_set_u_trns_slider)
...
        self.v_trns_button.clicked.connect(lambda: self.v_trns_force())
...
        self.v_trns_line_edit.textChanged.connect(self.text_set_v_trns_slider)
...
        self.uv_map_button.clicked.connect(lambda: self.apply_uv_map(uv_load=self.uv_dial_combobox))
...
        """
        url='https://atlas.bydeluxe.com/confluence/display/MCREA/Add+Geometry+Color+Tool'
...
        get_text_from_field = self.v_trns_line_edit.text()
        get_text_from_field = float(get_text_from_field)*10
        self.v_trns_sider.setValue(get_text_from_field)    def slider_set_v_tile_text(self):
        """
...
        self.uv_rot_slider.setValue(0.0)    def apply_uv_setting(self):
...
            create_shade_node = mc.shadingNode('blinn', asShader=True, n=name_shdr_nde)
...
        else:
            print ('nothing selected')
            return        checkerChecked = self.checker_checked
...
                mc.connectAttr( '{}.outUvFilterSize'.format(place
...
        """This is the dice rolling function for the RGB color setting
                Args:
                    type_color (str): 'Random', 'Grey' , 'Red' , 'Green' , 'Blue' ,'Teal' ,'Yellow' ,'Purple'
...
        """function for grabbing the median value of the current solid color swatch(to later change to grey value)
                Args:
                    shader_obj (str): "c_shoulders_0001_xhi_postTech_geo_shd"
                    type_color (str): "light"/"dark"/"sl_grey"

                Examples:
                    >>> object selection -> hypershade
                    if checkerChecked.isChecked():
                        for each_sel_shdr in mc.ls(sl=1):
                            self.slight_single_or_double(each_sel_shdr, "light")   
                            >>>>>>>>slight each color lighter by a random value of 0.425 increase in hue
                     mc.select(get_sel_obj, r=1)
        """        
...
        """function that takes the curent colors of the solid or checker and edits value for darkening, lightening or grey scale
                Args:
                    shader_obj (str): "c_shoulders_0001_xhi_postTech_geo_shd"
                    type_color (str): "light"/"dark"/"sl_grey"

                Examples:
                    >>> mc.getAttr("c_shoulders_0001_xhi_postTech_geo_ckr.color1")[0]
                    (.35562, .52186, .95315 )
                    >>> self.gamble_slight(("sl_grey", (.35562, .52186, .95315 ))
                    (.45826, .45826, .45826 ) 
                    >>>  mc.setAttr("c_shoulders_0001_xhi_postTech_geo_ckr.color1", .45826, .45826, .45826, type= "double3")
                    >>>  mc.getAttr("c_shoulders_0001_xhi_postTech_geo_ckr.color2")[0]
                    (.566265, .258952, .258952)
                    >>>  = self.gamble_slight(("sl_grey", (.566265, .258952, .258952 ))
                    (.38596, .38596, .38596 )
                    >>>  mc.setAttr("c_shoulders_0001_xhi_postTech_geo_ckr.color2", .38596, .38596, .38596, type= "double3")
                """             
...
        """This is the dice roll function for the editing values functions(slight, contrast, lighten)"""
        """function that takes current float tuple values, gambles a new value
        and adds or subtracts or finds a median (slight grey scale) balance to edit the color with
                Args:
                    instruction to perform : contrast_type (str): "sl_grey/dark/light"
                    list[tuple]: list of float values

                Returns:
                    list[tuple]: list of float values to set attr with

                Examples:
                >>>>contrast_type = "sl_grey" perform action: subtract random from first color value. add random to second  color value.
                >>>>get_clr_value = (.566265, .258952, .258952 )
                if contrast_type == "sl_grey"
                    >>>> random.uniform(.566265, .258952) = .35849
                    return  = (.35849, .35849, .35849 )  
                """          
...
        """function that takes current float tuple values, gambles a new value within a 0.25 range
        and adds and subtracts alternatively from the color attributes within the  
        checkered color node in order to enhance or reverse the value for better visibility
                Args:
                    instruction to perform : contrast_type (str): "enhance"/"reverse"
                    list[tuple]: list of two float values gathered from checker color attributes

                Returns:
                    list[tuple]: list of two new float values to set attr with

                Examples:
                >>>>contrast_type = "enhance" perform action: subtract random from first color value. add random to second  color value.
                >>>>get_shad_val_1 = (.566265, .258952, .258952 )
                >>>>get_shad_val_2 = (.210843, .187179, .187179 )
                if contrast_type == "enhance"
                    >>>> random.uniform(0.0,0.25) = .15
                    >>>> get_first = (.566265 -.15, .258952 -.15, .258952 - .15 )
                    >>>> random.uniform(0.0,0.25) = .7
                    >>>> get_second =(.210843 + .7, .187179 + .7, .187179 + .7 )
                    return get_first = (.566250, .258937, .258937 ) , get_second = (.210850, .187186, .187186 )  
                """        
        if contrast_type == 'enhance':
...
        """function that takes the curent colors of the checker and edits value for contrast type
                Args:
                    contrast_type (str): "enhance"/"reverse"

                Examples:
                    >>> (.566265, .258952, .258952 ) = mc.getAttr("c_shoulders_0001_xhi_postTech_geo_ckr.color1")[0]
                    >>> (.210843, .187179, .187179) = mc.getAttr("c_shoulders_0001_xhi_postTech_geo_ckr.color2")[0]
                    (.566250, .258937, .258937 ) , (.210850, .187186, .187186 ) = self.checker_contrast_gamble("enhance", (.566265, .258952, .258952 ), (.210843, .187179, .187179))
                    >>>  mc.setAttr("c_shoulders_0001_xhi_postTech_geo_ckr.color1", .566250, .258937, .258937, type= "double3")
                    >>>  mc.setAttr("c_shoulders_0001_xhi_postTech_geo_ckr.color2", .210850, .187186, .187186, type= "double3")
                """
        u_tile_value, v_tile_value, UV_rotnum, u_trns_value, v_trns_value =self.uv_collection()
        get_sel_obj = mc.ls(sl=1)
        mc.hyperShade (get_sel_obj[0], smn=1)
...
        """window that launches from the save/load window to save a gamut currently in selection"""
        getScenePath=mc.file(q=1, location=1)
        getPathSplit=getScenePath.split('/')
        folderPath='\\'.join(getPathSplit[:-1])+'\\color_gamut\\'
...
        fieldBucket=[]
        objname_file_node=str(newfolderPath)
        filebucket = [os.path.join(dirpath, name) for dirpath, dirnames, files in os.walk(objname_file_node) for name in files if name.lower().endswith('.txt')]
...
        self.get_file_name=mc.textField(h=25, p='listBuildButtonLayout', text=objname_file_node)
        mc.button (label='Save', w=90, p='listBuildButtonLayout', command = lambda *args:self.saveSelect(fileName=mc.textField(self.get_file_name, q=1, text=1)))            
        mc.button (label='Load', p='listBuildButtonLayout', command = lambda *args:self.loadSelect(mc.optionMenu(self.fileDropName, q=1, v=1), mc.textField(self.get_file_name, q=1, text=1)))
        mc.button (label='Refresh', p='listBuildButtonLayout', command = lambda *args:self.refresh_text(mc.textField(self.get_file_name, q=1, text=1)))
        mc.button (label='Open folder', w=60, p='listBuildButtonLayout', command = lambda *args:self.opening_folder(printFolder=mc.textField(self.get_file_name, q=1, text=1)))
        mc.showWindow(window)
    def refresh_text(self, file_text):
        menuItems = mc.optionMenu(self.fileDropName, q=True, ill=True)
        if menuItems:
            mc.deleteUI(menuItems)
        # getPathSplit=mc.textField(self.get_file_name, q=1, text=1)
        # print getPathSplit
        files, getPath, newfolderPath, filebucket=self.getWorkPath(file_text)   
        mc.optionMenu(self.fileDropName, e=1)
        for each in filebucket:
            mc.menuItem(label=each, parent=self.fileDropName)
...
    def opening_folder(self, printFolder):
        folderPath='\\'.join(printFolder.split("/")[:-1])+"\\"        
...
        if "Linux" in OSplatform:
            newfolderPath=re.sub(r'\\',r'/', folderPath)
            os.system('caja "%s"' % newfolderPath)


    def saveSelect(self, fileName):   
        """This is saving color gamut function - saves out a .txt file for reapplying the same gamut to the same character in a different shot
                Args:
                    fileName (str): '/jobs/vfx_thunder/ldk/ldk0010/TASKS/cfx/maya/scenes/color_gamut/uv_color.txt'

                Examples:
                       #####get the selected object list
                    >>>>get_sel_obj=mc.ls(sl=1, fl=1)
                    "EmilyA1Tech:l_sclera_hi_tech_geo"
                       #####get the shaders from the selected objects
                    >>>>mc.hyperShade (get_sel_obj[0], smn=1) #gets the select
                    "EmilyA1Tech:l_sclera_hi_tech_geo_shd"
                       #####parse each shader from selected
                    for each_sel_shdr in mc.ls(sl = 1):
                        >>>>get_item_name = each_sel_shdr.split('_shd')[0]
                            #####determines the type as a "checker"
                        >>>>if mc.objExists('{}_ckr'.format(get_item_name)) == True:
                                ######get the checker colors
                            >>>>chkcl1 = mc.getAttr('{}_ckr.color1'.format(get_item_name))
                            (0.6126819849014282, 0.679564356803894, 0.6622092723846436)
                            >>>>chkcl2 = mc.getAttr('{}_ckr.color2'.format(get_item_name))
                            (0.3248291313648224, 0.1579962521791458, 0.8833042979240417)
                                ######get the UV positions                            >>>>u_tile_value = mc.getAttr('{}_p2dt.repeatU'.format(get_item_name))
                            1.0
...
                            "EmilyA1Tech:l_sclera_hi_tech_geo_ckr"
                               #####build the string format
                            >>>>obj_string_line = '{}|{}<{}<{}>{}>{}>{}>{} , '.format(name_chkr_node, 
                                    chkcl1, 
                                    chkcl2,
                                     u_tile_value, 
                                     v_tile_value, 
...
                                     v_trns_value, 
                                     UV_rotnum)
                            "EmilyA1Tech:l_sclera_hi_tech_geo_ckr|[(0.6126819849014282, 0.679564356803894, 0.6622092723846436)]<[(0.3248291313648224, 0.1579962521791458, 0.8833042979240417)]<1.0>1.0>0.0>0.0>0.0"
                                ######save the string
                            >>>>inp.write(obj_string_line)
                            "/jobs/vfx_thunder/ldk/ldk0010/TASKS/cfx/maya/scenes/color_gamut/uv_color.txt"
                Returns:
                    new file in /jobs/vfx_thunder/ldk/ldk0010/TASKS/cfx/maya/scenes/color_gamut/uv_color.txt
                    contains text:
                    "EmilyA1Tech:l_sclera_hi_tech_geo_ckr|[(0.6126819849014282, 0.679564356803894, 0.6622092723846436)]<[(0.3248291313648224, 0.1579962521791458, 0.8833042979240417)]<1.0>1.0>0.0>0.0>0.0"         
        """        get_sel_obj=mc.ls(sl=1, fl=1)   
        if len(get_sel_obj)>0:
...
                    v_trns_value, 
                    UV_rotnum)
                inp.write(obj_string_line)            elif mc.objExists('{}_file'.format(get_item_name)) == True: 
...
                v_trns_value = mc.getAttr('{}_p2dt.offsetV'.format(get_item_name))
                UV_rotnum = mc.getAttr('{}_p2dt.rotateUV'.format(get_item_name))
                obj_string_line = '{}|{}<{}>{}>{}>{}>{} , '.format(name_file_node, 
                    uv_collectionimage, 
                    u_tile_value, 
                    v_tile_value, 
                    u_trns_value, 
...
        inp.close()   
        print ('saved as {}'.format(newfileName))    def loadSelect(self, printFolder, textfield): 
        get_path = textfield+'/'+printFolder
        if os.path.exists(get_path):
            pass
        else:
            print ('{} does not exist'.format(get_path))
            return
        List = open(get_path).readlines()
        create_dict = {}
        for aline in List:
            a_list = list(aline)
            a_list = aline.split(' , ')
...
                            create_shade_node = mc.shadingNode('blinn', asShader=True, n=shdr_name)
                            mc.hyperShade(assign=str(create_shade_node))
                            lst_sg_node=[create_shade_node]
                            set_name='techanim_textures' 
                            if mc.objExists(set_name):
                                pass
                            else:
                                mc.sets(n=set_name, co=3)                            mc.sets(lst_sg_node, add=set_name)
                            mc.select(scnobject)
                            mc.hyperShade(assign=str(create_shade_node))
...
                        mc.setAttr('{}.offsetU'.format(placement_node_p2dt), float(plc_get[2]))
                        mc.setAttr('{}.offsetV'.format(placement_node_p2dt), float(plc_get[3]))
                        mc.setAttr('{}.rotateUV'.format(placement_node_p2dt), float(plc_get[4]))                    else:
...
                                mc.delete(placement_node_p2dt)                                       
                            s = eval(str(listp1[1]))
                            print (listp1[0].split('_shd')[0]+'Shape_shd')
                            if mc.objExists(listp1[0].split('_shd')[0]+'Shape_shd') == True:
                                shade_name = listp1[0].split('_shd')[0]+'Shape_shd'
                                mc.setAttr('{}.color'.format(shade_name), s[0][0],s[0][1], s[0][2], type='double3')
                            elif mc.objExists(listp1[0]) == True:
                                mc.setAttr('{}.color'.format(listp1[0]), s[0][0],s[0][1], s[0][2], type='double3')
                            else:
                                print ('{}.color unable to change'.format(listp1[0]))
                                pass
                        else:
