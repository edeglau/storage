...
        self.L_P_button = QtWidgets.QPushButton("load and play")        self.open_button = QtWidgets.QPushButton("open folder")
        self.set_button.clicked.connect(lambda:  self.set_button_function(make_new_content = self.fieldText.text() ))
        self.back_button.clicked.connect(lambda:  self.back_button_function(make_new_content = self.fieldText.text() ))
        self.load_button.clicked.connect(lambda: self.load_button_function(self.fieldText.text()))
        self.load_hair_button.clicked.connect(lambda: self.load_hair_function(self.fieldText.text()))
        self.load_hair_single_button.clicked.connect(lambda: self.load_hair_single_function(self.fieldText.text()))
        self.load_cloth_button.clicked.connect(lambda: self.load_cloth_function(self.fieldText.text()))
        self.L_P_button.clicked.connect(lambda: self.L_P_button_function(self.fieldText.text()))
        self.open_button.clicked.connect(lambda: self.open_folder_button_function(self.fieldText.text()))
...
        print "refactor request: find a way to import geomtry cache without using pymel"
        if len(mc.ls(sl=1))<1:
...
                        mc.connectAttr(createdCacheFileNode+".outCacheData[0]", hairbit+".positions",                         mc.connectAttr("time1.outTime", createdCacheFileNode+".time", f=1)   
...
                        mc.setAttr(createdCacheFileNode+".sourceEnd", getHiRange)
                        mc.setAttr(createdCacheFileNode+".inRange", 1)
                        mc.connectAttr(createdCacheFileNode+".inRange", hairbit+".playFromCache", f=1)                        mc.connectAttr(createdCacheFileNode+".outCacheData[0]", hairbit+".positions", f=1)
...
        getHiRange = str(getHiRange)        if len(getHairSel)>0:
...
                        shape = hairbit
                        getLowRange = str(getLowRange)
                        getHiRange = str(getHiRange)                        createdCacheFileNode = mc.createNode("cacheFile", n=presetName+"Cache1")
                        print "created cachefile "+presetName+"Cache1"
                        mc.setAttr(createdCacheFileNode+".cachePath", filepath, type = "string")
                        print "set attr: "+createdCacheFileNode+".cachePath to "+filepath
                        mc.setAttr(createdCacheFileNode+".cacheName", presetName, type = "string")
...
                        print "connected attr: "+createdCacheFileNode+".outCacheData[2] to "+hairbit+".positions"
                        mc.connectAttr("time1.outTime", createdCacheFileNode+".time", f=1)           
                        print "connected attr: time1.outTime to "+createdCacheFileNode+".time"        else:
            for value in createdict:
                for hairbit in getHairSel:
                    filexml=str(filepath)+value
                    grabName = hairbit.split(":")[-1]
