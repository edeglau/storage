...
class get_set_sel_val(QtWidgets.QWidget):
    ...
    self.connect(self.ctrl_button, SIGNAL('clicked()'), lambda: self.build_ctrl_annot_one(trgt_ctrlrs))
...
class get_val_frm(QtWidgets.QWidget):
    ...
    def initUI(self, get_val_hi, def_val, trgt_obj, attr_channel):
        ...
        self.connect(self.sel_button, SIGNAL('clicked()'), lambda: self.build_annot(def_val, trgt_obj, attr_channel))
...
class annot_range_win(QtWidgets.QMainWindow):
    ...
    def initUI(self):
        ...
        self.connect(self.annot_sel_button, SIGNAL('clicked()'), lambda: self.sel_obj_vtx_annot())
        ...
        self.connect(self.auto_annot_button, SIGNAL('clicked()'), lambda: self.dealers_choice())
        ...
        self.connect(self.colour_annot_button, SIGNAL('clicked()'), lambda: self._change_anot_colors())
        ...
        self.connect(self.mrph_annot_button, SIGNAL('clicked()'), lambda: self.test_morph())
        ...
        self.connect(self.bsp_annot_button, SIGNAL('clicked()'), lambda: self.test_blendShape())
        ...
        self.connect(self.ctrl_annot_button, SIGNAL('clicked()'), lambda: self.ctrlr_annot())
        self.set_annot_button = QtWidgets.QPushButton("Test Set")
        # self.set_annot_button.clicked.connect(lambda: self.set_annot())
        self.connect(self.set_annot_button, SIGNAL('clicked()'), lambda: self.set_annot())
        ...
        self.connect(self.attr_annot_button, SIGNAL('clicked()'), lambda: self.test_attr())
        ...
        self.connect(self.all_attr_annot_button, SIGNAL('clicked()'), lambda: self.test_attr_controllers())
        self.sel_annot_button = QtWidgets.QPushButton("Test Selected")
        # self.sel_annot_button.clicked.connect(lambda: self.test_sel())
        self.ocnnect(self.sel_annot_button, SIGNAL('clicked()'), lambda: self.test_sel())
        ...
        self.ocnnect(self.recon_annot_button, SIGNAL('clicked()'), lambda: self.retape_to_selection())
        ...
        self.ocnnect(self.mk_annot_button, SIGNAL('clicked()'), lambda: self.make_arbitrary_title())
        ...
        self.ocnnect(self.mk_pubcam_button, SIGNAL('clicked()'), lambda: self.publishable_cam())
    ...
    def _change_anot_colors(self):
        ...
        if len(getgrp) == 0:
            ...
            print
            "annotations not present in scene"
            return
        for each in getgrp:
    ...
    def cam_constraint(self, tape_thing_to_cam):
        if mc.objExists("*:*.cameraPreset") == True:
            ...
            getNode = \
            [(mc.listRelatives(item, ap=1, type='transform')[0]) for item in mc.ls(type='camera') if 'persp' in item][0]
            try:
                selcnst = mc.parentConstraint(getNode, tape_thing_to_cam, mo=0)
                mc.rename(selcnst, 'annot_duct_tape_par')
            except:
                print
                "skipping {}".format(tape_thing_to_cam)
                pass
    ...
    def retape_to_selection(self):
        ...
        if mc.objExists("annot_loc_trn") == True:
            print
            "Label exists -  will skip creating it"
            print
            'taping label to selected'
            try:
    ...
    def ctrlr_annot(self):
        ...
        try:
            ...
            print
            "No heirarchy found from selected"
            pass
    ...
    def animate_function(self, get_frames, cust_dict, fnd_dir, trgt_ctrlrs):
        ...
        if len(cust_dict) < 1:
            print
            "need to pick a transform from the interface for scale and plane to animate in"
            return
        else:
            pass
    ...
    def test_attr(self):
        ...
        try:
            ...
            print
            "select attribute on slected object"
            return
    ...
    def test_blendShape(self):
        ...
        try:
            ...
            if mc.nodeType(get_bsp_obj[0]) != "blendShape":
                ...
                get_bsp = [(items) for shapes in get_Shapes for items in
                           mc.ls(mc.listHistory(shapes), typ='blendShape')]
            else:
                get_bsp = get_bsp_obj
            ...
            print
            "cannot find bsp target weights"
            return
    ...
    def test_morph(self):
        ...
        try:
            ...
            print
            "cannot find morph target weights"
            return
    ...
    def build_anim_singles(self, get_val,
        ...
        try:
            ...
            print
            "cannot add {} to set".format(sel_obj)
            pass
        print
        def_type
