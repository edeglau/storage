...
#        global makeAttr   
        window = cmds.window(winName, title=winTitle, tbm=1, w=500, h=600)
    ...
    or value.
        ...
        * Step 8(optional): press "Apply All" will change all attributes in
        ...
        findAttr=cmds.textField( text="Enter name EG:'translate'")
        cmds.button (label='Fetch Attribute Name',  bgc=[0.55, 0.6, 0.6], command = lambda *args:self._find_att(getName=cmds.textField(findAttr, q=1, text=1)))
        cmds.text(label="Search:", align="left", w=50)
        ...
        findAttrObj=cmds.textField(text="Enter name EG:'translate'")
        ...
        cmds.button (label='Filter Sel by Att Name',  bgc=[0.55, 0.6, 0.6], p='findbyattrButtonLayout', command = lambda *args:self._filter_att_obj(getName=cmds.textField(findAttrObj, q=1, text=1)))
        cmds.button (label='Filter by Att + Value',  bgc=[0.55, 0.6, 0.6], p='findbyattrButtonLayout', command = lambda *args:self._filter_att_value(getFirstattr=cmds.textField(findAttrObj, q=1, text=1), values=cmds.textField(valueAttrObj, q=1, text=1)))
        ...
        cmds.button (label='Apply Value Sel', p='listBuildButtonLayout2', w=100, command = lambda *args:self.apply_att_callup_sel(makeAttr=cmds.textField(makeAttrObj, q=1, text=1), attrName=cmds.textField(findAttrObj, q=1, text=1)))
        cmds.button (label='Key selected', p='listBuildButtonLayout2', w=100, command = lambda *args:self.key_att_callup_sel(makeAttr=cmds.textField(makeAttrObj, q=1, text=1), attrName=cmds.textField(findAttrObj, q=1, text=1)))
    ...
    def apply_att_callup_all(self, makeAttr):
        ...
        if menuItems:
            for each in menuItems:
                ...
                getChangeAttr=cmds.getAttr(getThing)
                try:
                    makeAttr=float(makeAttr)
                except:
                    print "Field must have number"
    ...
    def apply_att_callup_sel(self, makeAttr, attrName):
        ...
        for each in cmds.ls(sl=1, fl=1):
            try:
                ...
                print "Field must have number"
            try:
                cmds.setAttr(each+'.'+getAttr, makeAttr)
            except:
                pass
        # self.count_attr_output_obj(getChangeAttr)
    ...
    def _drop_selV1(self, getdrop):
        menuItems = cmds.optionMenu(self.objAtt, q=True, ils=True)
        collectTheThings=[]
        for each in menuItems:
            getThing=cmds.menuItem(each, q=1, label=1)   
            collectTheThings.append(getThing.split('.')[0])
    ...
    def _filter_att_objV0(self, getName):       
        if "," in getName:
            getName=getName.split(", ")
        else:
            getName=[getName]
        ...
        if menuItems:
            cmds.deleteUI(menuItems)       
        cmds.optionMenu(self.objAtt, e=1)
        for item, each in map(None, cmds.ls(sl=1), getName):
            # print item+'.'+each
            # if cmds.objExists(item+'.'+each):
            try:
                cmds.menuItem(label=item+'.'+each, parent=self.objAtt)  
            except:
                pass
    ...
    def _find_value_obj(self, values):
        Attrs=[(objectItem+'.'+attrItem)  for objectItem in cmds.ls ('*') for attrItem in cmds.listAttr (objectItem, w=1, a=1, s=1,u=1) if cmds.getAttr(objectItem+'.'+attrItem)==float(values)]
        menuItems = cmds.optionMenu(self.objAtt, q=True, ill=True)
        if menuItems:
            cmds.deleteUI(menuItems)       
        getListAttr=sorted(Attrs)
    ...
    def _filter_att_value(self, getFirstattr, values):
        ...
        #     values=int(values)  
        # print values      
        # Attrs=[(attrItem) for thing in cmds.listAttr (cmds.ls('*'), w=1, a=1, s=1,u=1) if cmds.ls('*.'+attrItem)==values]
        # menuItems = cmds.optionMenu(self.objAtt, q=True, ill=True)
        # if menuItems:
        #     cmds.deleteUI(menuItems)       
        # getListAttr=sorted(Attrs)
    ...
    def _find_value_objV1(self, getFirstattr, values):
        try:
            values=float(values)
        except:
            values=int(values)        
